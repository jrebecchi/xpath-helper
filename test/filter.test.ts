...
describe('Filter', () => {
  it('- and', () => {
    const h1Path = new XPathHelper()
      .getElementByTag("h1", filter.and(filter.valueContains('motherfudging'), filter.valueContains('website')))
  ...
  it('- or', () => {
    const h1Path = new XPathHelper()
      .getElementByTag("h1", filter.valueContains('motherfudging').or(filter.valueEquals('motherfudging')));
    const h1 = findByXpath(h1Path.toString());
    expect(h1).not.toBeNull();
    expect(h1?.textContent?.includes('motherfudging')).toBeTruthy();
  });
  it('- empty', () => {
    ...
    let h1Path = new XPathHelper()
      .getElementByTag("h1", aFilter);
    let h1 = findByXpath(h1Path.toString());
    expect(h1).toBeNull();
    aFilter.empty();
    h1Path = new XPathHelper()
      .getElementByTag("h1", aFilter);
  ...
  it('- hasAttribute', () => {
    const bodyPath = new XPathHelper()
      .getElementByTag("body", filter.hasAttribute('data-new-gr-c-s-check-loaded'))
    const body = findByXpath(bodyPath.toString());
    expect(body).not.toBeNull();
  });
  it('- attributeContains', () => {
    const bodyPath = new XPathHelper()
      .getElementByTag("body", filter.attributeContains('data-new-gr-c-s-check-loaded', '8'))
    const body = findByXpath(bodyPath.toString());
    expect(body).not.toBeNull();
  });
  it('- attributeEquals', () => {
    const bodyPath = new XPathHelper()
      .getElementByTag("body", filter.attributeEquals('data-new-gr-c-s-check-loaded', '8.884.0'))
    const body = findByXpath(bodyPath.toString());
    expect(body).not.toBeNull();
  });
  it('- attributeNotEquals', () => {
    const bodyPath = new XPathHelper()
      .getElementByTag("body", filter.attributeNotEquals('data-new-gr-c-s-check-loaded', 'toto'))
    const body = findByXpath(bodyPath.toString());
    expect(body).not.toBeNull();
  });
  ...
  it('- attributeGreaterThan', () => {
    const liPath = new XPathHelper()
      .getElementByTag("li", filter.attributeGreaterThan("data-number", 24))
    const li = findByXpath(liPath.toString());
    expect(li).not.toBeNull();
  });
  ...
  it('- valueContains', () => {
    const liPath = new XPathHelper()
      .getElementByTag("li", filter.valueContains('Stuff doesn\'t weigh a ton (in fact it\''))
    const li = findByXpath(liPath.toString());
    expect(li).not.toBeNull();
  });
  it('- valueEquals', () => {
    const liPath = new XPathHelper()
      .getElementByTag("li", filter.valueEquals(20))
    const li = findByXpath(liPath.toString());
    expect(li).not.toBeNull();
  });
  ...
  it('- valueNotEquals', () => {
    const liPath = new XPathHelper()
      .getElementByTag("li", filter.valueGreaterThan(14).and(filter.valueNotEquals(20)));
    const li = findByXpath(liPath.toString());
    expect(li).not.toBeNull();
  ...
  it('- valueLessThan', () => {
    const liPath = new XPathHelper()
      .getElementByTag("li", filter.valueLessThan(16))
    const li = findByXpath(liPath.toString());
    expect(li).not.toBeNull();
  });
  ...
  it('- valueGreaterThan', () => {
    const liPath = new XPathHelper()
      .getElementByTag("li", filter.valueGreaterThan(19))
    const li = findByXpath(liPath.toString());
    expect(li).not.toBeNull();
  });
  ...
  it('- get', () => {
    ...
    expect(p?.textContent?.includes('You probably build websites using vim')).toBeTruthy();
  });
  it('- getFirst', () => {
    const pPath = new XPathHelper()
      .getElementByTag('body')
