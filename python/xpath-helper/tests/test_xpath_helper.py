...
def test_get_child_by_svg_tag(html_doc):
    g_path = XPathHelper().get_element_by_svg_tag("g", filter.attribute_equals(
        "id", "Layer1")).get_child_by_svg_tag("path", filter.attribute_equals("id", "Shape"))
    elements = html_doc.xpath(str(g_path))
    assert len(elements) != 0

    g_path2 = XPathHelper().get_element_by_svg_tag("g", filter.attribute_equals(
...
def test_get_ancestor_by_tag(html_doc):
    ul_path = XPathHelper().get_element_by_tag(
...
def test_get_ancestor_by_svg_tag(html_doc):
    g_path = XPathHelper().get_element(filter.attribute_equals("id", "Shape8")
...
def test_get_ancestor_or_self_by_tag(html_doc):
    ul_path = XPathHelper().get_element_by_tag("a", filter.attribute_contains("href",
...
def test_get_ancestor_or_self_by_svg_tag(html_doc):
    g_path = XPathHelper().get_element(filter.attribute_equals("id", "Shape8")).get_ancestor_or_self_by_svg_tag(
        "g",
...
def test_get_following_by_tag(html_doc):
    li_path = XPathHelper().get_element_by_tag("i", filter.value_equals("almost")).get_following_by_tag(
        "li",
    ...
    a_path = XPathHelper().get_element_by_tag("i", filter.value_equals("even more")
...
def test_get_following_sibling_by_tag(html_doc):
    li_path = XPathHelper().get_element_by_tag("i", filter.value_equals("almost")).get_following_sibling_by_tag(
        "li",
        filter.value_contains("Doesn't load mbumive images or scripts.")
    )
    elements = html_doc.xpath(str(li_path))
...
def test_get_following_sibling(html_doc):
    li_path = XPathHelper().get_element_by_tag("li", filter.attribute_less_than(
...
def test_get_preceding_by_tag(html_doc):
    i_path = XPathHelper().get_element_by_tag(
        "li",
        filter.value_contains("Doesn't load mbumive images or scripts.")
    ...
    i_path2 = XPathHelper().get_element_by_tag("a", filter.value_equals("ARPANET")
...
def test_get_preceding(html_doc):
    a_path = XPathHelper().get_element(
        filter
...
def test_get_preceding_by_svg_tag(html_doc):
    rect_path = XPathHelper().get_element_by_svg_tag("svg", filter.attribute_equals("width", "298px")).get_preceding_by_svg_tag(
        "rect",
        filter
        .attribute_less_than("width", 640)
        .and_condition(filter.attribute_greater_than_or_equal_to("width", 620))
...
def test_get_preceding_sibling(html_doc):
    li_path = XPathHelper().get_element_by_tag("li", filter.value_greater_than_or_equal_to(20)).get_preceding_sibling(
...
def test_get_preceding_sibling_by_svg_tag(html_doc):
    rect_path = XPathHelper().get_element_by_svg_tag("path", filter.attribute_contains("d", "m423")).get_preceding_sibling_by_svg_tag(
    "rect",
    filter
      .attribute_less_than("width", 640)
      .and_condition(filter.attribute_greater_than_or_equal_to("width", 620))
