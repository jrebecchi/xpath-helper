...
def test_empty(html_doc):
    ...
    h1_path = XPathHelper().get_element_by_tag("h1", aFilter)
    elements = html_doc.xpath(str(h1_path))
    assert len(elements) == 0
    aFilter.empty()
    h1_path = XPathHelper().get_element_by_tag("h1", aFilter)
...
def test_attribute_less_than(html_doc):
    li_path = XPathHelper().get_element_by_tag("li", filter.attribute_less_than("data-number", 21)
                                               )
...
def test_attribute_less_thanOrEqualsTo(html_doc):
    li_path = XPathHelper().get_element_by_tag("li", filter.attribute_less_than_or_equal_to("data-number", 20)
                                               )
    elements = html_doc.xpath(str(li_path))
    assert len(elements) != 0
...
def test_attribute_greater_than(html_doc):
    li_path = XPathHelper().get_element_by_tag("li", filter.attribute_greater_than("data-number", 24)
                                               )
    elements = html_doc.xpath(str(li_path))
    assert len(elements) != 0
...
def test_attribute_greater_than_or_equal_to(html_doc):
    li_path = XPathHelper().get_element_by_tag("li", filter.attribute_greater_than_or_equal_to("data-number", 25)
                                               )
    elements = html_doc.xpath(str(li_path))
    assert len(elements) != 0
...
def test_value_contains(html_doc):
    li_path = XPathHelper().get_element_by_tag("li", filter.value_contains("Stuff doesn't weigh a ton (in fact it'")
                                               )
    elements = html_doc.xpath(str(li_path))
    assert len(elements) != 0
...
def test_value_equals(html_doc):
    li_path = XPathHelper().get_element_by_tag("li", filter.value_equals(20)
                                               )
    elements = html_doc.xpath(str(li_path))
    assert len(elements) != 0
...
def test_value_less_than(html_doc):
    li_path = XPathHelper().get_element_by_tag("li", filter.value_less_than(16)
                                               )
    elements = html_doc.xpath(str(li_path))
    assert len(elements) != 0
...
def test_value_less_thanOrEqualsTo(html_doc):
    li_path = XPathHelper().get_element_by_tag("li", filter.value_less_than_or_equal_to(15)
                                               )
    elements = html_doc.xpath(str(li_path))
    assert len(elements) != 0
...
def test_value_greater_than(html_doc):
    li_path = XPathHelper().get_element_by_tag("li", filter.value_greater_than(19)
                                               )
    elements = html_doc.xpath(str(li_path))
    assert len(elements) != 0
...
def test_get(html_doc):
    p_path = XPathHelper().get_element_by_tag(
        "body"
...
def test_get_last(html_doc):
    p_path = XPathHelper().get_element(filter.attribute_equals(
